...
def questiongroup(type, score_area, usage, ques_type, document, s):
	if score_area == 1:
		...
		p = document.add_paragraph()
		p.add_run('%s' % str(ques_type)).bold = True
		p.paragraph_format.space_before = Pt(8)  # 段前距
		p.paragraph_format.space_after = Pt(8)  # 段后距
...
def set_para_omml(para, omml):
	...
	'''
	p = para._p
	# oMath = p.first_child_found_in("m:oMath")
		...
		if li:
			...
			if math:
				...
				for j in range(len(math)):
					...
					set_para_omml(p, root)  # 将omml公式按xml节点的形式添加到当前段落的标签下
					# para = p._p
					# oMath = OxmlElement("m:oMath")
					# para.append(oMath)
					# oMath.extend(root)
			...
			for c in r.cells:
				for para in c.paragraphs:
					...
					if re_image.match(td_array[i]):
						...
						root = ET.fromstring(str(omml))
						set_para_omml(para, root)  # 将omml公式按xml节点的形式添加到当前段落的标签下
					else:
...
if __name__ == '__main__':
	path = r'C:\Users\j20687\Desktop\11.json'
