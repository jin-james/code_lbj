...
def read_word(file):
	...
	# print(paras)
	# print(questions)
	Questions, knowledge = word2html(end_para_no, QueStyle_para_no, questions, style, paras)  # 得到各题信息的汇总
	# print(Questions)
...
def word2html(end_para_no, QueStyle_para_no, questions, style, paragraphs):
	...
	re_know = re.compile(r'\[-题目序号-\]\d{1,3}\[-知识点-\]', re.S | re.M)
	# index = []  # 各组合题型数目
	# index.append(0)
	...
	for p in range(0, (len(QueStyle_para_no) - 1)):
		...
		for item in paragraphs:
			...
			if QueStyle_para_no[p] <= i < stop:
				...
				if "【题文】" in para_str:
					if knowledge_str:
						knowledge.append(knowledge_str.replace('<p>', '').replace('</p>', ''))
				...
				if "【结束】" in para_str:
					...
					knowledge_str = ""
					title_no += 1
				end_ = questions['question'][title_no] if questions['question'][title_no] != -1 else \
					questions["knowledge"][-1]
				if end_para_no[title_no - 1] < i < end_:
					knowledge_str += ' ' + re.sub(re_know, '', para_str)
				if end_ == questions["knowledge"][-1]:
					str_ = ""
					for jj in paragraphs[end_-1: -1]:
						str_ += ' ' + jj
					knowledge.append(re.sub(re_know, '', str_.replace('<p>', '').replace('</p>', '')))
			i += 1
...
def combine_question(Questions, opt_head):
	...
	:param Questions:所有题目的信息的汇总
	:return:组合题的单独处理
	"""
	re_subq = re.compile(r'小题\d{1,3}】(.*?)【|小题\d{1,3}】(.*?)<', re.S | re.M)
	re_sub_str = re.compile(r'^\d{1,3}.*?】', re.S | re.M)
	re_know = re.compile(r'\[-题目序号-\]\d{1,3}\[-知识点-\]', re.S | re.M)
	for i in range(len(Questions)):
		...
		if style_p in COMBINATION_QUESTIONS:
			...
			answer_str = ques.get('answer', '').replace('&nbsp;', '')
			# print("answer_str{}".format(answer_str))
			...
			print(combine_ques_list_len, len(combine_ques_list), combine_ques_list)
			# if len(group) == combine_ques_list_len:
			for j in range(combine_ques_list_len):
				sub_option = []
				...
				for k in range(len(sub_option)):
					...
					if "D" in value and k == 2:
						print("试卷{}第{}题的第{}个选项标签可能标注不规范，请按“A.”或“A、”的样式标注".format(style_p, j + 1, 4))
						break
					else:
						if key != opt_head[k]:
							print("试卷{}第{}题的第{}个选项标签可能标注不规范，请按“A.”或“A、”的样式标注".format(style_p, j + 1, k + 1))
							break
				# if key != opt_head[k]:
				# 	print("试卷{}第{}题的第{}个选项标签可能标注不规范，请按“A.”或“A、”的样式标注".format(style_p, j + 1, k + 1))
				# 	break
			...
			# else:
			# 	# raise MarkError("试卷试题类型可能标注不规范，请检查")
			...
			combine['answer'] = answer_str
			combine['analysis'] = analysis
...
if __name__ == '__main__':
	...
	file = r'C:\Users\j20687\Desktop\初中历史单选题1000题对应知识点.docx'
	...
	for q in range(len(questions)):
		i += 1
		questions[q]['knowledge'] = knowledge[q]
		print(questions[q])
	print(i)
	# with open(r"C:\Users\j20687\Desktop\know_map_ques.json", "w") as f:
	# 	json.dump(Exam, f)
	# print("加载入文件完成...")
