...
SUBJECTIVE_QUESTIONS = ["主观题", "计算题", "实验题", "作图题", "实验探究题", '解答题']  # 可能存在图片需要打印在答题卡上的题型
...
def read_word(file):
    ...
    # print(paragraphs)
    end_para_no, QueStyle_para_no, questions, exam_name, subject, style, paras = read_doc4para_no(doc, table_para, paragraphs)
    # print(paras)
    # print(questions)
...
def read_doc4para_no(doc, table_para, paras):
    ...
    for ll in range(len(style.keys())):
        if style[str(ll)] in ADD_BLANK_QUESTIONS:
            j = 0
...
def word2html(end_para_no, QueStyle_para_no, questions, style, paragraphs):
    ...
    for p in range(0, (len(QueStyle_para_no)-1)):
        ...
        style_p = style[str(p)]
        if '阅读理解' in style_p or '完形填空' in style_p:
            combine_question(questions, Questions, title_no, paragraphs, end_para_no, stop, style_p)
        else:
            for item in paragraphs:
                ...
                if QueStyle_para_no[p] <= i < stop:
                    ...
                    if "单选题" in style[str(p)] or "多选题" in style[str(p)]:
...
def combine_question(questions, Questions, title_no, paragraphs, end_para_no, stop, style_p):
...
if __name__ == '__main__':
    # file = r'C:\Users\j20687\Desktop\校本资源相关\test_paper_new\华南师大附中2018-2019学年第一学期八年级期中考试(数学)-教师用卷.docx'
    # file = r'C:\Users\j20687\Desktop\初中物理七年级题2-wzy.docx'
    # file = r'C:\Users\j20687\Desktop\校本资源相关\demo1.docx'
