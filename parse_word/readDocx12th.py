...
def read_word(file):
    ...
    # p1 = multiprocessing.Process(target=getMathml, args=('getMathml', proxy, q))
    # mmls_in_para = q.get()
    #
    # html = PyDocX.to_html(file)
    # p2 = multiprocessing.Process(target=get_img, args=('get_img_para', 'get_img_tbl', html, q))
    # # 启动子进程
    # p1.start()
    # p2.start()
    # # 等待fork的子进程终止再继续往下执行
    ...
    # images_in_para, images_in_table = q.get()
    ...
    # print(len(mmls_in_para), mmls_in_para)
    # print(images_in_para)
    html = PyDocX.to_html(file)
    mmls_in_para, images_in_para, images_in_table = asyncio.run(corun(proxy, html))
    # mmls_in_para = getMathml(proxy)  # 得到文档中编辑的公式的mathML格式的公式，便于前端显示
    # images_in_para, images_in_table = get_img(html)  # 得到文档中在表格和段落中的图片，返回的是图片的url
    ...
    paragraphs = get_para_html(proxy, images_in_para, mmls_in_para, table_html, table_para)  # 得到文档的段落信息，返回的是HTML标签
    # print(paragraphs)
    ...
    # return_dict = manager.dict()
    # jobs = []
    # 	# for i in range(5):  # 开启5个子进程执行word2html函数
    # 	# 	p = Process(target=word2html, args=(end_para_no, QueStyle_para_no, questions, style, paras, return_dict))  # 实例化进程对象
    # 	# 	p.start()
    # 	# 	jobs.append(p)
    # 	#
    # 	# for i in jobs:
    # 	# 	p.join()
    # Questions = return_dict
...
async def corun(proxy, html):
    task1 = asyncio.create_task(getMathml(proxy))  # 模拟执行1秒的任务
    task2 = asyncio.create_task(get_img(html))  # 模拟执行2秒的任务

    print(f'started at {time.strftime("%X")}')
    # 等待两个任务都完成，两个任务是并行的，所以总时间两个任务中最大的执行时间
    mmls_in_para = await task1
    images_in_para, images_in_table = await task2
    print(f'finished at {time.strftime("%X")}')
    return mmls_in_para, images_in_para, images_in_table


async def getMathml(proxy):
    '''
    遍历xml，找到m:oMath
    ...
    for xml in proxy:
        ...
        if omml != []:
            for ss in omml:
                ...
                ommls = (bytes(string, encoding='utf-8'))
                await omml2mml(ommls, transform, mmls)
    # n = 5
    # length = len(ommls)
    # threads = []
    # for i in range(n):
    #     sub_ommls = ommls[math.floor(i / n * length):math.floor((i + 1) / n * length)]
    #     t = threading.Thread(target=omml2mml, args=(sub_ommls, transform, mmls))
    #     threads.append(t)
    #     t.start()
    # for t in threads:
    #     t.join()
...
async def omml2mml(ommls, transform, mmls):
    dom = ET.parse(BytesIO(ommls))
...
def get_para_html(proxy, images_in_para, mmls, table_html, table_para):
    """
...
def read_doc4para_no(doc, table_para, paras, ispaper=True):
    ...
    re_blank = re.compile('\s+', re.S | re.M)
    ...
    for p in range(len(para)):
        ...
        if re_sub_content.findall(text_str):
            ...
            sub_test = re.sub(re_blank, "", sub_test)
            if sub_test:
                # print(text_str)
                raise BaseException("组合题型中可能存在“【小题x】”标签未置于行首位置，请检查并更正。")
        if "【选项】" in text_str:
            option.append(i)
        if "【答案】" in text_str:
            answer_test = text_str.split("【答案】")[0]
            if answer_test:
                raise BaseException("题目中可能存在“【答案】”标签未置于行首位置，请检查并更正。")
            answer.append(i)
        if "【解析】" in text_str:
            analysis.append(i)
        if "【结束】" in text_str:
            ...
            if end_test:
                raise BaseException("题目中可能存在“【结束】”标签未置于行首位置，请检查并更正。")
    ...
    if '0' in flag:
        ...
        if ispaper:
            if errno:
                raise BaseException("第{}题的标签可能存在错误，请检查。".format(nn))
            else:
                raise BaseException("试卷试题类型可能标注不规范或“【选项】”标签未添加全，"
    ...
    if QueStyle_para_no[0] > question[0] or QueStyle_para_no[0] == -1:
        raise BaseException("试卷可能存在题型标注错误，请按“一、单选题”的样式标注，请检查。")
...
def parano_compare(question, answer, analysis, end_para_no):
    ...
    for i in range(len(question)):
        ...
        if not (que_para < ans_para < ana_para < end_para):
            raise BaseException("试卷第{}题的标签可能存在错误，请按“题文，选项（若有），答案，解析，结束”的顺序标注，请检查。".format(i))
    all_para_no.append(-1)
    for j in range(len(all_para_no) - 1):
        if all_para_no[j] == all_para_no[j + 1]:
            m, n = divmod(j, 4)
            number = m + 1 if n else m
            raise BaseException("试卷第{}题可能存在定位标签块同行，请将两个标签置于不同行，请检查。".format(number))
...
def word2html(end_para_no, QueStyle_para_no, questions, style, paragraphs):
    ...
    title_no = 0  # 题目序号
    # ii = 0  # 组合题型计提序号
    re_no = re.compile(r'【题文】\d{1,3}\.|【题文】\d{1,3}、|\d{1,3}\.【题文】|\d{1,3}、【题文】'
                       , re.S | re.M)
                    ...
                    for j in range(len(option)):
                        ...
                        if style[str(p)] in MULTIPLE_QUESTION:
                            if "E" in value and j == 4:
                                raise BaseException("试卷{}第{}题的第{}个选项标签可能标注不规范，请按“A.”或“A、”的样式标注，请检查。".
                                                    format(style[str(p)], title_no, 5))
                            elif "F" in value and j == 5:
                                raise BaseException("试卷{}第{}题的第{}个选项标签可能标注不规范，请按“A.”或“A、”的样式标注，请检查。".
                                                    format(style[str(p)], title_no, 6))
                            else:
                                if key != opt_head[j]:
                                    raise BaseException("试卷{}第{}题的第{}个选项标签可能标注不规范，请按“A.”或“A、”的样式标注，请检查。".
    ...
    for i in range(len(Questions)):
        ...
        if style_p in COMBINATION_QUESTIONS:
            ...
            for ques_ss in combine_ques_p:
                ...
                if len(title_num) > 1:
                    raise BaseException("组合题型中可能存在小题标注块同行，请将两个标签置于不同行， 请检查。")
            ...
            for ss in range(len(combine_ques_list) - 1):
                ...
                # print(pre_no, post_no)
                if pre_no and post_no:
                    if pre_no == post_no:
                        raise BaseException("组合题型中可能存在小题题号名称重复，请检查。")
                # print(pre_no[0][0], post_no[0][0])
                # if pre_no[0] and post_no[0] and (
                # 		int(pre_no[0][0]) > int(post_no[0][0]) or int(pre_no[0][0]) + 1 < int(post_no[0][0])):
                # 	print(pre_no, post_no, combine_ques)
                # 	raise BaseException("组合题型中可能存在小题题号排序有误，请检查。")
            ...
            if len(group) == combine_ques_list_len:
                for j in range(len(group)):
                    ...
                    if re_sub_string:
                        ...
                        raise BaseException("组合题型的小题可能存在标签不规范，请按“【小题1】”的样式标注， 请检查。")
            else:
                raise BaseException("组合题型中可能存在小题的题干标签与答案标签不统一，请统一小题与之对应答案的标签块，请检查。")
...
if __name__ == '__main__':
    ...
    # file = r'C:\Users\j20687\Desktop\校本资源相关\test_paper_new\初中英语九年级题库-综合150-20200302413.docx'
