...
from nile.utils.constants import CHOICE_QUESTIONS, FILLIN_QUESTIONS, COMBINATION_QUESTIONS
...
def write_option(ques_no, choice_len, option_len, break_len, break_no, document):
...
def write4answersheet(answer_sheet_data):
    ...
    choice_len_sum = choice_len  # 选择题总数，包括组合题中的小选择题
    choice_start_num = choice_group[0].get('q_no') if choice_group else 0  # 选择题开始的题号
    option_len = [len(v['options']) for v in choice_group if v.get('options')]
    ...
    combine_len = len(combine_question) # 组合题的个数
    com_opt = [len(v['options']) for v in combine_question if v.get('options')]  # 每个组合题的最多选项数目
    combine_subques_len = [v['subques_len'] for v in combine_question if v['subques_len']]  # 小题的个数
    cs_num = combine_question[0].get('q_no') if combine_question else 0  # 组合题开始的题号
    break_len, break_no = 0, 0
    if combine_question:
        ...
        if choice_start_num > cs_num:
            # 组合题在选择题前面
            break_no = choice_len_sum
            break_len = choice_len_sum-choice_len
        else:
            # 选择题在组合题前面
            break_no = cs_num
            break_len = choice_len
    ...
    for ques in subjective_question:
        ...
        pic_num += 1
    qus_range_choice.append(choice_start_num+1)
    qus_range_choice.append(choice_len)
    if combine_question:
        qus_range_choice.append(cs_num+1)
        qus_range_choice.append(cs_num + choice_len_sum-choice_len)
    qus_range_fillin.append(fs_num+1)
    qus_range_fillin.append(fs_num + fillin_len)
    qus_range_eassy.append(ss_num + 1)
    qus_range_eassy.append(ss_num + subjec_len)
    fill_no = 0
    is_fillin_breakpage = False
    ...
    if choice_len_sum > 150:
        ques_no = write_option(ques_no, 60, option_len, break_len, break_no, document)
        document.add_page_break()
        paper_num += 1
        ...
        for n in range(pre_no):
            paper_num += 1
            add_barcode(document, paper_num + 1, flag)
            ques_no = write_option(ques_no, 90, option_len, break_len, break_no, document)
            document.add_page_break()
        ques_no = write_option(ques_no, rest, option_len, break_len, break_no, document)
        ...
        if choice_len_sum > 60:
            ...
            paper_num += 1
            add_barcode(document, paper_num + 1, flag)
            ques_no = write_option(ques_no, else_choice, option_len, break_len, break_no, document)
            writeline(document, black_font)
            ...
            ques_no = write_option(ques_no, choice_len_sum, option_len, break_len, break_no, document)
            writeline(document, black_font)
            choice_page = paper_num + 1
            ques_no, fill_no, num, paper_num, is_fillin_breakpage = choice0_60(fillin_len, choice_len_sum, fs_num, document,
                                                                              paper_num, flag)
            paper_num += num
            fillin_page = paper_num
    ...
    if is_fillin_breakpage and fillin_len:
        if fill_no == 0:
            ...
            for n in range(nn):
                ...
                ques_no, paper_num = sub_subjective(n, cs_num, paper_num, document, flag, is_add, pic_in_sub)
    ...
    if choice_len_sum:
        if fillin_page >= choice_page:
            d1, d2 = divmod(choice_len_sum, 90)
            if d2 > 45:
                for n in range(d1):
...
def create_paper_file(paper, data, fmt, size, usage):
    try:
        ...
        hash_code = r.hash
        models.ExamRelatedPaper.save_hash(paper=paper, type=size, hash_code=hash_code, usage=usage)
